---
name: bio-population-genetics-fine-mapping
description: "Perform locus-level fine mapping with explicit LD-reference and credible-set assumptions."
version: 0.1.0
tags: ["population genetics", "fine mapping", "credible sets", "LD"]
trigger_keywords: ["fine mapping", "credible set", "causal variants", "LD reference"]
metadata:
  openclaw:
    requires:
      bins: []
  ...
  bioskills:
    ...
    canonical_of:
      ...
      - ClawBio/ClawBio:ClawBio-main/skills/fine-mapping/SKILL.md
    depends_on:
      - bio-population-genetics-association-testing
---

# Population-genetic fine mapping

## Purpose / When To Use

- Perform locus-level fine mapping with explicit LD-reference and credible-set assumptions.
- Use this skill when the user needs population-genetic fine mapping in the context of population genetics.
...
- Require summary statistics, LD reference, and locus definitions before credible-set claims are made.

## Execution Path

- Keep locus preparation, LD assumptions, and posterior outputs clearly separated in the result.
...
- Review LD compatibility, credible-set size, and posterior concentration.
...
- Pause when LD reference and discovery cohort are mismatched in a way that breaks interpretation.
