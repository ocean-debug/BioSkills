---
name: bio-bulk-rna-seq-count-matrix-qc
description: "Inspect count-matrix quality, replicate concordance, and sample-level outliers before differential testing."
version: 0.1.0
tags: ["rna-seq", "qc", "counts", "pca"]
trigger_keywords: ["count matrix QC", "library size", "PCA", "replicate concordance"]
metadata:
  openclaw:
    requires:
      bins: []
...
- Inspect count-matrix quality, replicate concordance, and sample-level outliers before differential testing.
- Use this skill when the user needs bulk rna-seq count-matrix qc in the context of bulk rna seq.
...
- Check whether the matrix is raw counts, normalized expression, or pseudo-bulk before any recommendations.
...
- Highlight sample swaps, severe outliers, and metadata mismatches early.
...
- Do not continue if the matrix columns cannot be reconciled with the sample sheet.

## Related Skills

- bio-bulk-rna-seq-batch-correction
